...
# gtkui.py
#
# Copyright (C) 2009 SharkByte <SharkByte@Sharks.com>
#
...
class GtkUI(GtkPluginBase):
    def enable(self):
        ...
        component.get("Preferences").add_page("SearchDB", self.glade.get_widget("prefs_box"))
        self.plugin_manager.register_hook("on_apply_prefs", self.on_apply_prefs)
        self.plugin_manager.register_hook("on_show_prefs", self.on_show_prefs)
        self.tbar_separator = self.plugin_manager.add_toolbar_separator()
        self.tbar_search = self.plugin_manager.add_toolbar_button(self.search,
            label="Searching", stock=gtk.STOCK_FIND, tooltip="Looking at DB")

    def disable(self):
        component.get("Preferences").remove_page("SearchDB")
        self.plugin_manager.deregister_hook("on_apply_prefs", self.on_apply_prefs)
        self.plugin_manager.deregister_hook("on_show_prefs", self.on_show_prefs)
    ...
    def on_apply_prefs(self):
        log.debug("applying prefs for SearchDB")
        config = {
            "test":self.glade.get_widget("txt_test").get_text()
        }
        client.searchdb.set_config(config)

    def on_show_prefs(self):
        client.searchdb.get_config().addCallback(self.cb_get_config)
